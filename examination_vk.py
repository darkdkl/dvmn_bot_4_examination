...
def handle_solution_attempt(event, vk_api,keyboard):
    ...
    else:send_message(event, 'Не верный ответ или команда', keyboard, vk_api)
    

def help_luser(event, vk_api,keyboard):
    redisbase.delete(str(event.user_id)+'question_is_asked')
    send_message(event, 'Вот тебе правильный ответ: '+str(redisbase.get(event.user_id), 'utf-8') +
                     '\n Что бы продолжить нажми "Новый вопрос"', keyboard, vk_api)
    ...
    try:
        ...
        for event in longpoll.listen():
            if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                ...
                elif event.text == 'Новый вопрос':handle_new_question_request(event, vk_api,keyboard)
                elif event.text == 'Сдаться':help_luser(event, vk_api,keyboard)
                elif event.text == 'Мой счет':get_user_score(event, vk_api,keyboard)
                else:handle_solution_attempt(event, vk_api,keyboard)
